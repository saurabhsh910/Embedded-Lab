...
/** @brief STat ready check */
#define STAT_READY 0x200
...
/** @brief physical address of AUX_MU_STAT_REG */
#define AUX_MU_STAT_REG_REG (volatile uint32_t *)(MMIO_BASE_PHYSICAL + 0x215064)
...
/** @brief AUX_MU_STAT_REG register set */
static volatile uint32_t* aux_mu_stat_reg = (volatile uint32_t*) AUX_MU_STAT_REG_REG;
...
void uart_init(void) {
  ...
  *aux_mu_lcr_reg = ENABLE_UART; //CHANGE TO ENABLE_UART IF UART IS GARBAGE
}
...
void uart_put_byte(uint8_t byte) {
  ...
  while((*aux_mu_lsr_reg & SEND_DONE) == 0) {
  }

  while((*aux_mu_stat_reg & STAT_READY) == 0){
  }
...
uint8_t uart_get_byte(void) {
  //*aux_mu_iir_reg = *aux_mu_iir_reg | CLEAR_SEND;
  *aux_mu_ier_reg = CLEAR_SEND;

  // Wait for receive FIFO to have stuff
  while( (*aux_mu_lsr_reg & RECEIVE_DONE) == 0){
  }
